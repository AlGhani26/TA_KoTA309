...
# Konfigurasi
INPUT_DIR = 'selected_images_gid_16bit_10'  # Folder gambar BGR-NIR 16-bit
OUTPUT_DIR = 'patch_images_gid_16_bit_10'  # Folder output
PATCH_SIZE = 256  # Ukuran patch
BAND_ORDER = ['Blue', 'Green', 'Red', 'NIR']  # Urutan band

def create_bgr_nir_patches(image_path, output_folder):
    """Membuat patch 256x256 dari citra BGR-NIR 16-bit"""
    try:
...
def main():
    ...
    print("Memulai proses pembuatan patch BGR-NIR 16-bit 256x256...")
    ...
    for filename in os.listdir(INPUT_DIR):
        if filename.lower().endswith(('.tif', '.tiff')):
            ...
            num_patches = create_bgr_nir_patches(image_path, OUTPUT_DIR)
            total_patches += num_patches
            print(f"  -> Berhasil membuat {num_patches} patch")
    
    print(f"\nSelesai! Total {total_patches} patch BGR-NIR 16-bit berukuran {PATCH_SIZE}x{PATCH_SIZE} dibuat di '{OUTPUT_DIR}'")
